            ...
            for row in reader:
                if len(row) >= 4:
                    try:
                        ...
                        disponibilidad = {k.replace('�', 'ó'): v for k, v in disponibilidad.items()}
                        aulas.append({
                            'nombre': row[0],
                            'capacidad': capacidad,
                            'edificio': row[2],
                        ...
                        print(f"Error: Valor inválido en la fila {row} del archivo {archivo}")
                else:
                ...
                if len(row) >= 12:
                    ...
                    try:
                        ...
                        if isinstance(profesores_lista, list):
                            profesores = ', '.join(f"{prof['nombre']} {prof['apellido']}" for prof in profesores_lista)
                    except (ValueError, SyntaxError):
                        pass
                    materias.append({
    ...
    for aula in aulas:
        ...
        for dia, disponibilidad_horaria in disponibilidad_aula.items():
            for hora, disponible in enumerate(disponibilidad_horaria, start=8):
                if disponible:
                    horarios_disponibles_aulas[aula['nombre']][dia].append(f"{hora}")
        ...
        for hora_inicio, hora_fin in horas_separadas:
            for aula, aulas_disponibles in horarios_aulas.items():
                for dia, horas in aulas_disponibles.items():
                    horas_aula = [int(horas) for horas in aulas_disponibles[dia]]
...
def asignar_materias_a_aulas(materias, horarios_profesores, horarios_aulas, edificio_predefinido):
    ...
    for materia in materias:
        ...
        for profesor_nombre in profesores_separados:
            aulas_con_disponibilidad = verificar_disponibilidad(profesor_nombre, horarios_profesores, horarios_aulas)
            if aulas_con_disponibilidad:
                for aula in aulas_con_disponibilidad:
                    aula_nombre = aula['Aula:']
                    aula_edificio = next((a['edificio'] for a in aulas if a['nombre'] == aula_nombre), None)
...
def escribir_sugerencias(sugerencias, archivo):
    with open(archivo, 'w', newline='', encoding='ISO-8859-1') as csvfile:
        ...
        writer.writerow(['Materia', 'Profesor', 'Aula', 'Dia', 'Hora inicio', 'Hora fin', 'Edificio'])
...
def copiar_archivo_aulas(archivo_origen, archivo_destino):
    try:
        ...
        print(f"Error: No se pudo copiar el archivo {archivo_origen} a {archivo_destino}: {e}")
...
def crear_sugerencia_asignacion(archivo_aulas, archivo_destino_aulas, archivo_materias, archivo_destino_materias):
    ...
    with open(archivo_destino_materias, 'w', newline='', encoding='ISO-8859-1') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Codigo Guarani', 'Nombre', 'Carrera', 'Año', 'Cuatrimestre', 'Profesores', 'Alumnos Esperados', 'Horas Frente Curso', 'Comisiones'])
...
sugerencias = asignar_materias_a_aulas(materias, horarios_disponibles_profesores, horarios_disponibles_aulas, 'Anasagasti II')
...
crear_sugerencia_asignacion('Aulas.csv', 'sugerencia_asignacion.csv', 'Materias.csv', 'Materias_reordenadas.csv')
